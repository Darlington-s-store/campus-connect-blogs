...
const Login = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    const success = await login(email || indexNumber, password);
  ...
  const handleSetNewPassword = async (e: React.FormEvent) => {
    ...
    setIsSubmitting(true);
    const success = await setNewPassword(indexNumber, newPassword);
            ...
            <form onSubmit={handleLogin}>
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  <Label htmlFor="emailOrIndex">Email or Index Number</Label>
                  <Input
                    id="emailOrIndex"
                    placeholder="Email or Student Index Number"
                    value={email || indexNumber}
                    onChange={(e) => {
                      const value = e.target.value;
                      // If input contains only digits, it's an index number
                      if (/^\d*$/.test(value)) {
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    value={indexNumber}
                    onChange={(e) => setIndexNumber(e.target.value.replace(/[^0-9]/g, ''))}
                    required
                  />
                </div>
                <div className="space-y-2">
